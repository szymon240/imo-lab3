...
def swap_nodes_between_cycles_with_candidates(first_cycle, second_cycle, distance_matrix, candidate_edges):
    """
    Zamienia wierzchołki między cyklami, ograniczając do kandydatów.
    """
    n1, n2 = len(first_cycle) - 1, len(second_cycle) - 1
    for i in range(1, n1):
        # Pobierz kandydatów dla wierzchołka z pierwszego cyklu
        candidates = candidate_edges[first_cycle[i]]
        for j in range(1, n2):
            # Sprawdź, czy wierzchołek z drugiego cyklu jest kandydatem
            if second_cycle[j] in candidates:
                # Wprowadzenie krawędzi n1-n2
                new_first = first_cycle[:]
                new_second = second_cycle[:]
                new_first[i], new_second[j] = new_second[j], new_first[i]
...
def swap_edges_within_cycle_with_candidates(cycle, distance_matrix, candidate_edges):
    """
    Zamienia krawędzie w obrębie cyklu, ograniczając do kandydatów.
    """
    n = len(cycle) - 1
    for i in range(1, n - 1):
        # Pobierz kandydatów dla wierzchołka cycle[i]
        candidates = candidate_edges[cycle[i]]
        for j in range(i + 2, n):
            # Sprawdź, czy wierzchołek cycle[j] jest kandydatem
            if cycle[j] in candidates:
                # Wprowadzenie krawędzi n1-n2
                new_cycle = cycle[:]
                new_cycle[i:j + 1] = reversed(new_cycle[i:j + 1])
